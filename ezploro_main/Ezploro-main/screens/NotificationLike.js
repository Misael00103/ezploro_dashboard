...
export default function NotificationLike({ route }) {
  const { notification } = route.params;
  const { darkMode } = useTheme();
  return (
    <View style={[styles.container, darkMode && { backgroundColor: '#181A20' }]}> 
      <Ionicons name="heart-outline" size={48} color="#e53935" style={styles.icon} />
      <Text style={[styles.title, darkMode && { color: '#fff' }]}>Nuevo like</Text>
